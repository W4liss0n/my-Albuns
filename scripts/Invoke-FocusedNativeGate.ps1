param(
    [ValidateSet('focused-owned-dialogs', 'saved-original-close')]
    [string] $GateName = 'focused-owned-dialogs',
    [string] $OutputPath,
    [ValidateSet('all', 'external-copy-opening-owner', 'late-graphics-project-dialog', 'saved-original-close')]
    [string] $Scenario = 'all',
...
$isProjectClose = $GateName -eq 'saved-original-close'
if ($isProjectClose -ne ($Scenario -eq 'saved-original-close')) {
    throw 'The gate and selected scenario do not match.'
}
$runnerName = if ($isProjectClose) { 'Run-ProjectCloseGate.mjs' } else { 'Run-FocusedOwnedDialogGate.mjs' }
$retainedEvidenceName = if ($isProjectClose) { 'project-close-evidence' } else { 'focused-owned-dialog-evidence' }
...
$retainedEvidenceRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $scratchParent $retainedEvidenceName)
)
...
$runRoot = [System.IO.Path]::GetFullPath(
    (Join-Path `
        $scratchParent `
        "$GateName-$PID-$([guid]::NewGuid().ToString('N').Substring(0, 8))")
)
...
$previousNativeGateAuthorization = $env:MYALBUNS_NATIVE_GATE_AUTHORIZED

function Copy-NativeScenarioDiagnostics {
    Get-ChildItem -LiteralPath $runRoot -File | Where-Object {
        $_.Name -like 'webdriver-*.log' -or $_.Name -like 'failure-*' -or $_.Name -like 'project-close-*.json'
    } | ForEach-Object {
...
try {
    ...
    $env:MYALBUNS_NATIVE_GATE_AUTHORIZED = "1"
    $focused = Invoke-GateScopedCommand `
        -Scope $scope `
        -Name 'focused-native' `
        -FilePath $node `
        -Arguments @(
            (Join-Path $PSScriptRoot $runnerName),
            $workspaceRoot,
            $runRoot,
            $applicationPath,
    ...
    $projectCloseInvalid = $isProjectClose -and (
        -not $gate.projectClose.originalHostExited -or
        -not $gate.projectClose.copyHostAlive -or
        -not $gate.projectClose.copyResponsive -or
        -not $gate.projectClose.savedFilesUnchanged -or
        -not $gate.projectClose.independentIdentities -or
        -not $gate.projectClose.cleanCloseObserved -or
        -not $gate.projectClose.replacementGlobalReady
    )
    if (($gate.scenarios -join ',') -cne ($expectedScenarios -join ',') -or
        $externalCopyInvalid -or $graphicsInvalid -or $projectCloseInvalid -or -not $gate.cleanupCompleted) {
    ...
    $report = [ordered]@{
        ...
        projectClose = $gate.projectClose
        cleanup = [ordered]@{
    ...
    if ($null -eq $previousNativeGateAuthorization) {
        Remove-Item Env:MYALBUNS_NATIVE_GATE_AUTHORIZED -ErrorAction SilentlyContinue
    } else {
        $env:MYALBUNS_NATIVE_GATE_AUTHORIZED = $previousNativeGateAuthorization
    }
    if ($null -ne $scope) {
