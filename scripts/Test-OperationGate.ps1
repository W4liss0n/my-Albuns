...
$projectOpenScenarios = @('normal_close', 'owner_death')
$projectOpenRoots = [ordered]@{}
foreach ($scenario in $projectOpenScenarios) {
    $projectOpenRoots[$scenario] = [System.IO.Path]::GetFullPath(
...
$fixtureRoots = switch ($Suite) {
    ...
    'project_open' {
        @($projectOpenScenarios | ForEach-Object { $projectOpenRoots[$_] })
    }
}
...
function Assert-ProjectOpenScenarioName {
    ...
    if ($script:projectOpenScenarios -cnotcontains $Scenario) {
...
function Start-OperationProbeProcess {
    ...
    if ($hasProjectOpenScenario) {
        Assert-ProjectOpenScenarioName -Scenario $ProjectOpenScenario
        if ($Topology -cne 'independent') {
    ...
    $environment = [ordered]@{
        ...
        MYALBUNS_PROJECT_OPEN_PROBE_ROOT = $null
        MYALBUNS_PROJECT_OPEN_PROBE_FILE = $null
        MYALBUNS_PROJECT_OPEN_PROBE_SCENARIO = $null
    }
    if ($hasProjectOpenScenario) {
        $environment.MYALBUNS_PROJECT_OPEN_PROBE_ROOT = $ProbeRoot
        $environment.MYALBUNS_PROJECT_OPEN_PROBE_FILE = Join-Path `
            $ProbeRoot `
            'Projeto.myalbum'
        $environment.MYALBUNS_PROJECT_OPEN_PROBE_SCENARIO =
            $ProjectOpenScenario
    }
...
function New-ProjectOpenFixture {
    ...
    $payload = [System.Text.UTF8Encoding]::new($false).GetBytes(
        '{"schemaVersion":1,"projectId":"project-open-probe","revision":0}'
    )
    $stream = [System.IO.File]::Open(
        $path,
        [System.IO.FileMode]::CreateNew,
...
try {
    ...
    elseif ($Suite -eq 'project_open') {
        Test-StrictProjectOpenEventParser `
            -ProbeRoot $projectOpenRoots['normal_close']
    }
    else {
    ...
    elseif ($Suite -eq 'project_open') {
        $projectOpenResults = [ordered]@{}
        foreach ($scenario in $projectOpenScenarios) {
            $result = Invoke-ProjectOpenScenario `
                -Scenario $scenario `
                -ProbeRoot $projectOpenRoots[$scenario]
            $projectOpenResults[$scenario] = $result
            $checks.Add([ordered]@{
