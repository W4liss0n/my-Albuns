...
No modo de correção, não há mensagens soltas sobre as imagens. Ausência de rosto, falha de análise e erro ao preparar ou salvar aparecem somente no tooltip compartilhado da ação correspondente, com acesso por hover e foco de teclado. A mensagem identifica a foto afetada quando necessário e orienta a próxima ação; não exige um ícone de aviso separado. Uma ação indisponível que tenha uma explicação continua acessível para consultar o tooltip, sem executar por clique ou teclado. Erros recuperáveis preservam a possibilidade de tentar novamente. Textos de andamento como **Analisando rostos**, **Preparando correção** e **Aplicando correção** são dispensados; os controles bloqueados e o estado acessível de ocupação comunicam o processamento. A prévia dispensa instruções repetidas; não há ações nem faixa de mensagens no rodapé.
