...
A comparação ocupa duas metades com áreas de imagem de mesma altura útil: referência à esquerda e imagem a corrigir à direita. As ações são uma extensão da ferramenta **Abrir olhos**, no canto superior esquerdo da área de visualização. Ao entrar na correção, o mesmo ponto passa a oferecer **Fechar correção** e os outros ícones aparecem ao lado. O grupo flutua sobre a área do visualizador, sem barra de fundo, caixa ou rodapé. Os rótulos das fotos e as ferramentas mantêm espaço entre si, inclusive na janela estreita.
