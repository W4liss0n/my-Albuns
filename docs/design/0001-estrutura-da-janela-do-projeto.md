...
A única política de substituição pertence ao fluxo `Abrir Projeto`: depois que um arquivo existente ou um Projeto recente foi escolhido, a superfície de origem sai da área visível antes da janela de progresso. Se a abertura encontrar Recuperação ou uma Cópia externa somente leitura, essa mesma janela externa de progresso muda para a decisão correspondente sem expor conteúdo na Global nem criar ou mostrar a Janela do Projeto. O Host e a tentativa continuam correlacionados até o terminal; cancelar apenas o seletor nativo de destino retorna à decisão da Cópia externa, enquanto o botão `Cancelar` encerra e recolhe o Host pendente. O detalhamento e a recuperação em caso de falha pertencem ao design da [Tela de Boas-vindas](0002-tela-de-boas-vindas.md).

Uma falha gráfica tardia do Canvas não substitui a Janela do Projeto por uma página ou cartão de erro. Ela usa o mesmo sistema de diálogo externo pertencente à Janela do Projeto, bloqueia a proprietária e retira a autoridade operacional do Canvas. `Fechar Projeto` encaminha o fechamento pelo contrato já existente; se alterações não salvas exigirem decisão, a confirmação de fechamento ocupa o mesmo ciclo serializado e, caso o usuário cancele esse fechamento, o aviso gráfico volta a ser o owner ativo. Relatos repetidos atualizam uma única sessão lógica e não criam janelas duplicadas. `Novo Projeto`, a confirmação de `Informações do Álbum`, fechamento, Exportação e os demais avisos ou progressos seguem o padrão de proprietário visível e bloqueado.
...
- No Canvas contínuo, uma Lâmina de Página única conserva a mesma largura visual de uma Lâmina dupla. A Página ativa ocupa a metade correspondente ao seu lado canônico, enquanto a outra metade recebe um degradê neutro quente e suave que parte da borda externa clara e ganha profundidade em direção à dobra central, sem trama repetitiva. Essa metade não contém Página, conteúdo, Frames, Fotos ou guias técnicas e não participa da Exportação, mas integra a área de hover, foco, clique e Barra da própria Lâmina.
...
- Um Frame vazio usa preenchimento neutro liso e contorno fino, sem ícone. No Canvas interativo, a ação `Adicionar Foto` permanece centralizada e visível; as miniaturas da Grade omitem o rótulo para preservar a leitura na escala reduzida. O cursor permanece o padrão do sistema sobre qualquer Frame. Essa representação pertence à interface e não altera a Borda persistida do Frame.
...
- No Modo de edição, `Ctrl+A` ou `Editar > Selecionar tudo` seleciona todos os Frames da Lâmina isolada. Arrastar a partir da área vazia do Canvas seleciona os Frames tocados pela Caixa, inclusive parcialmente e em sobreposição; `Ctrl` acrescenta à seleção anterior. A prévia é confirmada ao soltar e `Esc` cancela sem sair da edição. Um clique vazio limpa a seleção e, com `Ctrl`, preserva-a. Selecionar continua permitido em Layout travado e não altera o Histórico. Campos de texto e Painel de imagens conservam seus próprios atalhos.
...
- No Modo de edição, na Lâmina de origem, o deslocamento efetivo de `Colar` é o menor entre o deslocamento visual desejado e o que mantém todo o conjunto na superfície ativa. Se nenhum deslocamento for viável, as cópias ficam na mesma posição, tornam-se a seleção atual e entram acima dos originais na Pilha visual por ordem determinística. Em outra Lâmina com a mesma superfície, preservam as posições originais.
- No Modo de edição, quando o destino é uma Página única e a superfície de origem é diferente, `Colar` mapeia proporcionalmente as posições e dimensões do conjunto inteiro para a Página ativa. As relações entre Frames, suas Fotos ou placeholders, a ordem, os estilos e os ajustes são preservados.
- No Modo de edição, ao colar de uma Página única em uma Lâmina dupla, o conjunto é mapeado proporcionalmente somente para a Página do mesmo lado lógico: direita na direita e esquerda na esquerda. A composição nunca é ampliada automaticamente para ocupar a Lâmina inteira.
- Nos dois modos, `Colar` reutiliza os vínculos com os mesmos Arquivos originais, sem copiar mídia ou Cache.
- Depois da colagem, o Modo de edição seleciona todas as novas ocorrências; o Modo normal seleciona somente a última ocorrência criada, preservando sua seleção simples. Uma seleção, navegação ou troca de modo feita durante a espera tem prioridade sobre essa seleção automática. A colagem e sua eventual reorganização de Layout geram uma única ação de Undo/Redo. O Modo de edição preserva a geometria sem reaplicar Layout.
...
`Informações do Álbum` reúne somente as configurações globais não visuais do Projeto. Controles editáveis e configurações ainda sem operação implementada preservam a aparência de campo; estas últimas são marcadas explicitamente como placeholder no código. Valores derivados somente de consulta, como `Dimensão da Página`, usam texto integrado à superfície do Painel.

`Dimensão da Lâmina` e `Dimensão da Página` formam grupos distintos, cada um com `Largura` e `Altura` apresentadas separadamente. `Unidade` usa um campo compacto ao lado do controle de DPI, sem reservar uma linha inteira do Painel.
