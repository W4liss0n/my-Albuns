...
Na aba Fotos, `Importar → Arquivos JPEG…` permite escolher vários Originais na mesma seleção. Cancelar não altera o Projeto. Durante a seleção e a inspeção, o botão indica `Importando…` e impede outra seleção simultânea. A fila existente ordena as alterações seguintes; a Exportação aguarda a conclusão da importação.
...
O comando de validação completa é `npm run validate`, com relatório local em [report.json](../../.tools/validation/report.json). A evidência visual fica em [report.html](../../.scratch/ui-acceptance/import-multiple-jpeg-final/report.html), com decisões no [relatório revisado](../../.scratch/ui-acceptance/import-multiple-jpeg-final/review-report.html). Capturas de componentes e testes de portas não são aceitação do seletor do Windows.
