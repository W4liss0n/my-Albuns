...
- A decisão de Recuperação pertence ao host e à Janela do Projeto que detêm a sessão. Exatamente um diálogo modal é apresentado sobre a superfície normal desse owner, sem trocar a rota ou substituir o conteúdo principal e sem migrar para a Global ou outra Janela. O diálogo possui backdrop, `role=dialog`, modalidade acessível, título associado, foco inicial deliberado, trap e restauração de foco; `Esc` executa o mesmo terminal de `Agora não`.
