...
- A faixa superior compartilhada centraliza `Configurações` e `Personalização` no espaço disponível à esquerda do controle `Modelo inicial` e mantém esse controle alinhado à direita durante as duas etapas. O foco desse controle composto usa um único contorno externo, sem bordas sobrepostas no campo ou na ação de salvar.
