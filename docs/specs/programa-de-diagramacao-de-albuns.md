...
1. Como pessoa diagramadora, quero receber um aviso quando uma gravação realmente falhar por falta de espaço, sem alertas preventivos nem bloqueios baseados em estimativas de tamanho.
...
1. Como pessoa diagramadora, quero encaixar Frames em bordas, centros e guias técnicas durante a edição, para alinhar a composição com precisão.
1. Como pessoa diagramadora, quero igualar larguras, alturas e espaçamentos durante o arraste, para repetir medidas sem ajustar cada uma manualmente.
1. Como pessoa diagramadora, quero usar o mesmo Espaço entre Frames nos snaps e no Gerador de Layouts, para manter uma referência única no Projeto.
1. Como pessoa diagramadora, quero suspender os snaps segurando `Ctrl` durante o arraste e tratar uma seleção múltipla como uma unidade, para manter controle sobre o gesto.
...
1. Como pessoa diagramadora, quero que cada Janela consulte o catálogo global ao abrir o Painel de Layouts ou receber foco, para ver mudanças persistidas sem exigir sincronização instantânea entre processos.
...
- A capa do cartão permanece um placeholder nesta versão. Uma capa real somente pode receber contrato depois das medições e da reavaliação arquitetural exigidas pelo ADR 0005 para previews persistidos de Lâmina; até lá, esta SPEC não define sua geração, persistência, transporte, esquema ou atualização.
...
- Escolhas simples são aplicadas e persistidas imediatamente, sem botão geral `Aplicar` ou `Salvar`. `Limpar cache` mantém sua confirmação e depois executa em segundo plano, sem diálogo de progresso nem cancelamento durante a execução.
...
- O aplicativo não consulta espaço livre nem estima tamanhos para apresentar alertas preventivos ou impedir o início de Cache e Exportação. Uma falha real de falta de espaço ou de cota na criação, gravação, finalização ou publicação gera uma orientação específica. No Cache, a preparação afetada falha sem modificar Projeto, Original ou geração anterior válida; avisos não se repetem por miniatura no mesmo Projeto aberto. Na Exportação, a falha respeita as regras de preparação e Publicação por arquivo e informa quando o Destino contém um conjunto parcialmente publicado. A mensagem não inicia limpeza automática.
...
- Campos, seletores e ações comuns indicam foco por teclado com uma mudança neutra e discreta, sem contorno azul. O contorno azul fica reservado a seleções espaciais do produto: escopo na prévia de `Personalização`, Lâminas e Frames no Canvas, Lâminas na Grade e mídias no Painel de imagens.

### Criação de Projeto

- `Novo Projeto` abre um fluxo do próprio aplicativo composto por exatamente duas etapas: `Configurações` e `Personalização`.
- `Novo Projeto` aparece como contexto no título da janela e não se repete na área de conteúdo.
- A faixa superior compartilhada centraliza `Configurações` e `Personalização` no espaço disponível à esquerda do controle `Modelo inicial` e mantém esse controle alinhado à direita durante as duas etapas. O foco desse controle composto usa somente uma mudança neutra de fundo, sem contorno azul ou bordas sobrepostas no campo e na ação de salvar.
- `Configurações` apresenta diretamente Unidade, Dimensão da Lâmina fechada, Sangria, Área de segurança, quantidade inicial de Lâminas, Resolução do Projeto e Configuração das extremidades; esses dois últimos grupos não ficam recolhidos.
...
- Toda entrada inválida mantém indicação visual no próprio controle e expõe sua mensagem como descrição acessível, sem ocupar espaço no layout. Quando um formulário passa do estado válido para o inválido, um Tooltip com o resumo abre automaticamente e permanece visível até todos os erros serem resolvidos ou ocorrer um clique fora da entrada que o ancora. Depois de dispensado, restam somente as indicações vermelhas nos controles. Clicar ou mover o foco do teclado novamente para qualquer entrada ainda inválida reabre o resumo ancorado nessa entrada; hover sozinho não o reabre. Uma nova ocorrência após o formulário voltar a ficar válido também pode abrir o Tooltip novamente. Mensagens iguais simultâneas no mesmo formulário são deduplicadas em um único resumo compartilhado. O Tooltip usa os tons claros de erro da paleta compartilhada e uma ponta visual voltada para o campo que o ancora, formando um balão sem superfície escura invertida. Mensagens que orientam a correção de medidas físicas usam a Unidade selecionada e o DPI corrente, convertendo limites internos de raster para o intervalo físico equivalente do campo em vez de apresentar pixels ou micrômetros como entrada. Em `Novo Projeto`, o foco vai para o primeiro campo inválido na ordem visual e o fluxo não abre um modal genérico de validação.
...
- O bloco `Frames` substitui checkbox e campos genéricos por um controle deslizante de espessura, no qual zero aparece como `sem borda`, três amostras de cor — branca, escura e dourada — e um controle deslizante de `Espaço entre Frames` expresso na Unidade escolhida em `Configurações`.
- `Espaço entre Frames` define uma medida física única do Projeto, compartilhada pelos snaps de espaçamento padrão e pelo Gerador de Layouts. Começa em `5 mm`, apresenta sua conversão na Unidade escolhida e atualiza a reprodução da etapa. O valor permanece no draft até a criação válida e passa a integrar o Projeto criado; cancelar não persiste a escolha nem cria Frames de demonstração no Álbum.
...
- A própria Lâmina oferece somente duas regiões clicáveis, lado esquerdo e lado direito; clicar em qualquer ponto do painel de prévia fora da Lâmina, inclusive cabeçalho, metadados e legenda, fixa `Ambos os lados`, e a região central da Lâmina não constitui um alvo separado.
- A seleção fixa mostra um contorno azul sólido desenhado integralmente para fora da superfície selecionada e mantém toda essa superfície com presença total e seus Frames com contraste forte; cada lado não selecionado recebe uma atenuação neutra translúcida sobre toda a sua superfície. O foco por teclado mostra um contorno azul-claro pontilhado e recuado, com contraste intermediário nos Frames que não pertencem à seleção.
- O hover reduz a atenuação somente de um lado que não pertença à seleção fixa e o cobre com uma tonalidade neutra compartilhada pelo tema, sem contorno e sem alterar o contraste individual dos Frames; um lado selecionado nunca recebe hover e, com `Ambos os lados` fixado, nenhuma pré-seleção aparece.
...
- Um splitter horizontal redimensiona Canvas e Painel de imagens. Sua aparência é uma linha divisória discreta de `1 px`, enquanto um alvo invisível sobreposto de `12 px` preserva hit-test por mouse/pen sem consumir espaço útil; hover, foco, cursor, ARIA, teclado e arraste permanecem operáveis. `Exibir > Painel de imagens` oculta ou restaura o Painel; ocultá-lo entrega a altura disponível ao Canvas.
...
- Um splitter vertical segue a mesma linha visual de `1 px` e alvo invisível sobreposto de `12 px`, separa a coluna de trabalho inteira do Painel contextual e permite ajustar a largura direita sem sobreposição.
...
- Cada Lâmina possui uma Barra própria sobreposta aos 40 px superiores de sua superfície, sem reservar espaço adicional nem alterar a escala automática do Canvas. A Barra pertence somente à interface, acompanha a largura visual da Lâmina, mantém altura e tipografia constantes na tela e nunca participa da Exportação.
- Fora do hover, a Barra permanece visualmente oculta. Ao entrar na Lâmina, ela aparece como uma superfície neutra quente translúcida com opacidade de `0,55`; fundo, divisor e controles usam contraste suficiente para permanecer legíveis mesmo sobre composições claras. A Numeração de Página fica centralizada em cada lado ativo, com ações compactas e o número da Lâmina alinhado à direita. Entrar na própria Barra eleva suavemente sua opacidade para `1`, e cada ação escurece individualmente sob o ponteiro; sair da Barra retorna a `0,55`, enquanto sair da Lâmina volta a ocultá-la sem alterar foco ou seleção.
...
- Clicar em uma área vazia limpa a seleção do elemento e retorna ao contexto do Álbum. Clicar na Grade de Lâminas ou usar as teclas físicas `←` e `→` navega; arrastar uma miniatura da Grade reordena; rolar o Canvas não altera o contexto.
- Toda ação explícita de navegação para uma Lâmina, inclusive Grade e as teclas físicas `←`/`→`, alinha o centro visual da Lâmina de destino ao centro horizontal da área útil do Canvas; não a alinha à borda esquerda. `←` percorre a Lâmina física anterior, `→` percorre a seguinte e ambas passam pelo catálogo público de comandos. Não existem botões permanentes de anterior/próxima nos cantos do Canvas; campos editáveis, diálogos, menus, Modo de edição e interações com semântica própria retêm ownership das teclas.
...
- `Design do Álbum` contém somente `Padrões visuais` (Background e Overlay) e `Padrão dos Frames` (cor e espessura da Borda, sendo a espessura zero a ausência dela, e o controle de Espaço entre Frames do Projeto).
...
- Em `Documento`, trocar Unidade converte imediatamente toda medida física visível na Janela do Projeto para a Unidade pendente, inclusive medidas apresentadas fora de `Informações do Álbum`, no cabeçalho e em `Design do Álbum`. A conversão altera somente a apresentação: os valores físicos continuam em micrômetros, nada persiste antes do `Aplicar` de `Informações do Álbum`, e `Design do Álbum` não se torna modificado nem habilita seu próprio `Aplicar` por causa dela. Descartar ou desmontar o formulário, ou trocar de Projeto, restaura a Unidade aplicada. Largura, altura e DPI permanecem pendentes até o `Aplicar` de `Informações do Álbum`, que apresenta tamanho físico e resolução finais e confirma todas as alterações não visuais atomicamente em uma ação de Undo/Redo.
- Cada entrada numérica de `Informações do Álbum` exibe um `X` integrado somente quando seu draft difere do último valor aplicado. Acioná-lo restaura apenas aquela entrada; medidas voltam ao valor aplicado na Unidade atualmente exibida, e uma conversão causada somente pela troca de Unidade não exibe `X` nas medidas.
...
- Background e Overlay ocupam uma linha cada, com o rótulo à esquerda e exatamente duas opções à direita, no tamanho das amostras de cor da criação de novo Projeto; a faixa não cresce com o acervo. O Background tem a amostra de cor e o quadrado de Decorativo; o Overlay tem `Sem Overlay`, que também limpa a escolha, e o quadrado de Decorativo. Um divisor separa a opção base do quadrado de Decorativo. Não há botão, nome de arquivo nem texto de estado ao redor da faixa, e o nome do Decorativo aplicado fica no Tooltip do quadrado. Clicar numa opção altera somente o draft visual até o `Aplicar` de `Design do Álbum`.
- O quadrado de Decorativo mostra o Decorativo aplicado no escopo e aparece tracejado quando vazio. Clicar nele abre um seletor flutuante ancorado na linha inteira, e não no quadrado, para não ultrapassar a borda direita da janela. O seletor repete a grade da aba `Decorativos` do Painel de imagens — mesma miniatura com moldura branca e sombra, mídia inteira sem corte, xadrez quando não há prévia. Escolher um Decorativo aplica ao draft e fecha o seletor; ponteiro fora ou `Esc` fecham sem alterar, devolvendo o foco ao quadrado.
- A última célula da grade é `Importar Decorativo`, tracejada e sem xadrez. Ela permanece indisponível enquanto os comandos de importação não tiverem porta de aplicação, com a mesma marcação usada no Painel de imagens.
- O seletor flutuante é a única superfície secundária admitida neste controle, e existe porque o padrão do Projeto se configura no formulário de `Design do Álbum`, não no Painel de imagens. Fica vedado o caminho inverso: `Design do Álbum` não lê a seleção do Painel de imagens nem depende de um alvo aguardando escolha. As faixas continuam sem aceitar arraste e sem importar arquivos.
- O indicador de selecionado da faixa usa a mesma cor de acento da Lâmina ativa e do card de mídia selecionado, na variante de borda, sem anel.
- `Padrão dos Frames` não repete uma prévia própria, porque a miniatura de `Padrões visuais` já mostra o efeito da Borda. O grupo traz o seletor de cor e a espessura na mesma linha e não oferece amostras de cor sugeridas, que se confundiam com o controle que abre o diálogo do sistema.
- A espessura dispensa alternador: o slider começa em zero, zero significa ausência de Borda e a leitura mostra `sem borda` nessa posição. Suas alterações permanecem no mesmo draft visual.
- `Espaço entre Frames` integra o draft de `Design do Álbum` e habilita seu `Aplicar` quando houver alteração válida. Antes da confirmação, somente a miniatura da seção usa o valor pendente. Aplicar atualiza a medida usada pelos próximos snaps e consultas do Gerador, sem reorganizar as composições existentes nem alterar as geometrias de Layouts já salvos. O valor confirmado participa de Undo/Redo, Salvamento e reabertura; não é parte do Estilo do Frame nem uma propriedade herdada por ocorrência.
...
- `Informações do Álbum` e `Design do Álbum` possuem, cada um, seu próprio botão `Aplicar` integrado ao cabeçalho da seção, sem reservar um rodapé vazio. Sem alterações, a ação é discreta e desabilitada; somente um draft válido a destaca e habilita. O primeiro confirma conjuntamente as alterações não visuais; o segundo confirma conjuntamente o draft visual e o Espaço entre Frames. Cada aplicação válida cria uma única ação de Undo/Redo.
...
- No Modo de edição, `Ctrl+A` e `Editar > Selecionar tudo` selecionam todos os Frames da Lâmina isolada, inclusive placeholders, sem alterar o Projeto ou o Histórico. Campos de texto e Painel de imagens conservam seus próprios atalhos.
- Arrastar a partir de uma área vazia do Canvas no Modo de edição, após o limiar de arraste da plataforma, desenha uma Caixa de seleção azul translúcida. Todos os Frames tocados pelo retângulo entram na seleção, inclusive os parcialmente cobertos ou sobrepostos. Sem modificador substitui a seleção; `Ctrl` acrescenta à seleção anterior, respondendo durante o gesto. Um clique vazio sem arraste limpa a seleção; com `Ctrl`, preserva-a.
- A Caixa destaca a seleção provisória durante o gesto e a confirma ao soltar. `Esc` cancela a Caixa e preserva a seleção anterior sem sair do Modo de edição; perda de foco, cancelamento do ponteiro ou mudança de Projeto, Lâmina ou composição também cancelam. Selecionar é permitido em Layout travado. No modo normal e no Painel de imagens não há Caixa de seleção.
...
- Movimento e redimensionamento diretos usam Snaps de Frames automaticamente no Modo de edição, sem botão de ativação. Segurar `Ctrl` durante o gesto suspende todos os snaps; soltar a tecla reavalia o encaixe a partir do ponteiro atual. `Ctrl` + clique conserva a regra de seleção.
- Os snaps de alinhamento usam bordas e centros de Frames externos à seleção, inclusive placeholders, bordas e centros das Páginas ativas, divisão e centro da Lâmina dupla, linha de corte e limites da Área de segurança. Guias técnicas desativadas não acrescentam alvos, e referências coincidentes são consolidadas.
- O Snap de dimensão iguala largura com largura e altura com altura de outro Frame, independentemente de sua posição. Alças laterais atuam somente no seu eixo; nos cantos, os eixos podem encaixar independentemente. `Shift` preserva a proporção, `Alt` preserva o centro e a combinação mantém ambas as restrições. Candidatos incompatíveis com essas regras, com a superfície ou com o tamanho mínimo são descartados.
- Os snaps de espaçamento funcionam no movimento e no redimensionamento, na horizontal e na vertical. Permitem repetir intervalos existentes entre Frames vizinhos, equilibrar os espaços entre dois vizinhos e atingir o Espaço entre Frames configurado no Projeto. A medida considera o espaço livre entre retângulos externos; sobreposições não são intervalos negativos de referência.
- Para snaps, a seleção múltipla é um único objeto: alinhamento usa as bordas e os centros da Caixa delimitadora; igualdade de dimensão usa sua largura e altura totais; espaçamento usa seus limites externos. Frames internos não produzem snaps individuais nem são alvos do próprio gesto. Movimento e redimensionamento continuam transformando todo o conjunto.
- A sensibilidade inicial adquire um encaixe a até `6 px` lógicos de correção na tela e conserva o alvo até exigir mais de `10 px`, medidos a partir do gesto livre. Essa distância é constante na tela em diferentes níveis de Zoom, Unidades e DPI de saída. O alvo válido mais próximo prevalece; a retenção evita alternância entre candidatos próximos.
- Em empate entre resultados diferentes, preferir alinhamento, dimensão, espaçamento padrão e espaçamento igual, nessa ordem, seguido de desempate geométrico estável. Com proporção preservada, escolher uma escala compatível com o gesto e não impor dois encaixes que exijam escalas diferentes.
- Guias de snap usam traços magenta de `1 px` constante na tela. Igualdade de dimensão e espaçamento exibem as medidas correspondentes na Unidade do Projeto. Mostrar somente o resultado alcançado, identificar sua referência e consolidar coincidências. As guias desaparecem ao terminar ou cancelar o gesto, suspender com `Ctrl` ou perder o alvo válido.
- O encaixe consolida somente a geometria final, sem vínculos que façam um Frame acompanhar futuras alterações de outro. Guias, cotas e estado de snap não pertencem ao Projeto, ao Histórico ou à Exportação. Gesto cancelado ou sem alteração efetiva não cria ação. Inserção de Fotos, colagem e aplicação de Layouts conservam suas próprias regras de geometria.
- O [contrato de Snap de Frames](../design/0033-snap-de-frames.md) detalha vizinhança, estabilidade, modificadores, indicação visual e cenários de aceite. A sensibilidade e as guias devem ser calibradas no Canvas real durante a implementação.
...
- No Modo normal, arrastar uma Foto sobre outro Frame troca o conteúdo sem entrar no Modo de edição. O destino pode estar na mesma Lâmina ou em outra Lâmina do mesmo Projeto, e a seleção simples atual é preservada.
- O gesto começa depois do limiar de arraste do Windows. Uma miniatura semitransparente da imagem inteira acompanha o ponteiro, preservando a proporção original e sem aplicar o recorte, Pan, Zoom, rotação ou espelhamento da ocorrência no Frame. A miniatura cabe em 80 × 60 pixels e fica a 6 pixels do ponteiro, respeitando os limites do Canvas; a composição de origem permanece no lugar. O Frame de destino válido recebe contorno azul destacado e uma leve tonalidade azul. O cursor permanece como mão fechada durante todo o arraste, sem alternar entre mover e bloqueado enquanto o destino é resolvido. Nas bordas do Canvas, a rolagem permite alcançar outras Lâminas. `Alt` + arraste continua reservado ao Pan.
- Soltar sobre a própria origem, área vazia, Página inativa ou fora do Canvas não altera o Projeto. `Esc`, perda de foco, cancelamento do ponteiro ou mudança da composição cancelam o arraste sem Histórico.
...
- Nos modos normal e de edição, `Editar > Copiar` e o atalho fixo do MVP `Ctrl + C` copiam a Seleção de Frames com geometrias, ordem relativa, Fotos ou placeholders, estilos e ajustes não destrutivos. No Modo normal, vale a seleção simples existente; no Modo de edição, a cópia pode conter vários Frames.
- Copiar não altera o Projeto nem cria Histórico. A seleção copiada permanece disponível ao navegar para outra Lâmina ou alternar os modos do mesmo Projeto. `Editar > Colar` e `Ctrl + V` usam a Lâmina isolada no Modo de edição; no Modo normal, usam sempre a Lâmina centralizada, mesmo que um Frame de outra Lâmina continue selecionado. No Modo normal, os Frames copiados são adicionados à Lâmina e o Layout é reorganizado automaticamente pela mesma regra da inclusão de Fotos, sem preservar suas posições anteriores. No Modo de edição, a colagem conserva sua geometria própria, sem reaplicar Layout.
- No Modo de edição, ao colar na Lâmina de origem, o deslocamento efetivo das novas ocorrências é o menor entre o deslocamento visual desejado e o deslocamento que ainda mantém todo o conjunto na superfície ativa. Se nenhum deslocamento for viável, as cópias permanecem na mesma posição, tornam-se a seleção atual e entram acima dos originais na Pilha visual por uma ordem determinística.
- No Modo de edição, quando o destino é uma Página única e a superfície de origem é diferente, posições e dimensões de todo o conjunto são mapeadas proporcionalmente para a Página ativa. Fotos ou placeholders, ordem, estilos e ajustes permanecem associados aos Frames resultantes.
- No Modo de edição, quando a origem é uma Página única e o destino é uma Lâmina dupla, o conjunto é mapeado proporcionalmente somente para a Página do mesmo lado lógico: direita permanece direita e esquerda permanece esquerda. A colagem não amplia o conjunto para a Lâmina inteira.
- Nos dois modos, a colagem reutiliza os mesmos vínculos externos, sem duplicar Arquivos ou Cache.
- A colagem e sua eventual reorganização de Layout constituem uma única ação de Undo/Redo. No Modo de edição, seleciona todas as novas ocorrências; no Modo normal, seleciona somente a última ocorrência criada, mantendo a seleção simples. Se o usuário mudar a seleção, navegar ou alternar o modo enquanto a colagem está pendente, sua escolha posterior é preservada.
...
- Com a Lâmina destravada, o seletor de quantidade do Painel de Layouts permite reduzir o total até a quantidade de Frames com Foto, sem exigir apagar placeholders antes. A prévia conserva todas as Fotos e, ao aplicar, remove somente os placeholders excedentes em uma única ação de Undo/Redo. Os Frames mantidos conservam ordem, estilos e ajustes de Foto. O cadeado precisa ser aberto antes de alterar a quantidade.
...
- Criação e exclusão substituem atomicamente a revisão do catálogo global. Cada Janela consulta a revisão vigente ao abrir o Painel de Layouts ou receber foco; não há botão de atualização manual, e broadcast imediato entre Janelas não é requisito do MVP.
...
- A decisão de Recuperação pertence à tentativa de abertura mantida pela Global/Boas-vindas e ao Host pendente correlacionado, antes da Janela do Projeto existir ou ficar visível. A mesma janela nativa externa e pertencente que mostra o progresso de abertura transiciona para exatamente um diálogo de decisão, sem substituir a rota da Global nem renderizar overlay no WebView do Projeto. O owner permanece bloqueado e estável; o diálogo possui `role=dialog`, modalidade acessível, título associado, foco inicial deliberado, trap e restauração de foco, e `Esc` executa o mesmo terminal de `Agora não`. Recuperar ou abrir a versão salva continua no mesmo Host e só então permite criar/exibir o Projeto; `Agora não`, fechamento, falha e cancelamento encerram deterministicamente a tentativa sem duplicar Sessão ou descartar o checkpoint por aproximação.
...
- A Recuperação não guarda o Histórico disponível. O orçamento do Histórico em memória é de 128 MiB por Projeto aberto, estimado pelo tamanho das versões guardadas, e não tem configuração manual na primeira versão.
...
- Durante essa decisão, tentativa repetida, fechamento, cancelamento e falha do Host têm cleanup determinístico: nenhuma escolha duplica Sessão ou Host, e o checkpoint só é descartado pelo terminal confirmado que já possui esse contrato.
...
- Frames e Fotos devem ser exercitados em diferentes proporções, rotações, espelhamento, Pan, Zoom, redimensionamento, troca de conteúdo com Foto ou placeholder e cópia/colagem simples ou múltipla na mesma Lâmina, entre Lâminas equivalentes e nos dois sentidos entre Lâmina dupla e Página única, verificando Preenchimento do Frame, deslocamento de colagem limitado, fallback sem deslocamento, mapeamento proporcional, preservação do lado lógico, reutilização dos vínculos, isolamento entre Projetos, seleção por Caixa no Modo de edição, cancelamento do gesto, respeito ao foco de `Ctrl+A` e atomicidade do Histórico.
- Layouts devem ser testados por compatibilidade, prioridade, identidade com sequência ordenada, atualização do catálogo ao abrir/reabrir/focalizar, mapeamento pela Pilha visual, preservação de conteúdo e estilo, criação de placeholders, travamento, `Delete` que preserva Frames travados, destravamento e diferenças entre modo normal e Modo de edição.
...
- Snaps devem cobrir alinhamento, dimensão e espaçamento em movimento e redimensionamento, seleção como uma unidade, oito alças, modificadores dinâmicos, tolerância constante na tela, retenção e troca de alvo, referências coincidentes, vizinhança, Frames vazios ou transparentes, restrições físicas e Layout travado. A validação inclui guias e cotas no Canvas real, criação e alteração do Espaço entre Frames compartilhado com o Gerador, cancelamento, atomicidade do Histórico e equivalência da geometria ao salvar, reabrir e exportar.
