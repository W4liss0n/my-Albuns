...
| Barra do Painel de imagens | Faixa de tom com linha inferior, sem sombra. Abas `Fotos` e `Decorativos` só com ícones (imagem e formas); a ativa usa a superfície da grade, traço azul de 2 px no topo e bordas laterais finas. `Importar` em texto discreto logo após as abas, com a única divisória da barra. Chips sem fundo; o ativo usa `--ui-accent-soft` e `--ui-accent-text`. `Ausentes` só aparece quando há arquivos ausentes na aba ou com o filtro ligado. Busca integrada de 160 px com linha inferior. | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md#painel-de-imagens) |
| Painel de layouts | Linha de tom com o título `Layouts com [n] quadros`: o número é o seletor, sublinhado e em grafite; abre uma grade de seis colunas com todas as quantidades, sem lista longa. Seções com títulos horizontais em frase, separadas por espaço. Na miniatura, estrela sozinha à esquerda; cadeado e lixeira numa cápsula clara translúcida à direita; todos só com ponteiro ou foco, exceto estados ativos em azul. | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md#barra-da-lâmina-e-painel-de-layouts) |
| Barras de título | 36 px em todas as janelas, nome em 13 px; a barra de menus do editor fica abaixo com 28 px e `Exportar` de 22 px. Marca sempre `myalbuns` em grafite, peso normal, nunca azul; só a Janela do Projeto a mostra na barra. Editor: `myalbuns · nome` e tamanho à esquerda, na fonte da interface; estado por extenso à direita com ponto (`Alterações não salvas` em cinza secundário, `Salvo` em cinza claro). | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md#diálogos-pertencentes-a-uma-janela) |
| Editor indisponível (modo seguro) | A própria tela de boas-vindas, sem cartão nem abas: ações de projeto e lote desabilitadas, `Configurações…` ativa, `modo seguro` na barra. No lugar dos recentes, título de 13 px com o motivo em frase simples, uma frase sobre o que ainda funciona e `Detalhes para o suporte` como lista plana selecionável (rótulo cinza, valor grafite). | [0002](../docs/design/0002-tela-de-boas-vindas.md#editor-indisponível) |
| Painel contextual | Cabeçalhos de seção e de contexto em faixa de tom (`--ui-surface-muted`), com linha só quando a seção está aberta. Grupos com título de subseção em frase (11 px, 650, cinza secundário), separados por espaço. Sem maiúsculas nem monoespaçada: números e unidades na fonte da interface, algarismos alinhados, unidade em cinza. `Ambos os lados` é título de grupo no painel; no Novo projeto, nota discreta de alcance (11 px, cinza). | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md#painel-contextual-direito) |
| Confirmação de Informações do álbum | Só aparece quando aplicar remove personalizações de uma extremidade ou muda a proporção das lâminas. Título, uma frase por consequência no tom do texto de apoio e a nota discreta `Você pode desfazer tudo de uma vez.`; sem lista de campos nem valores `anterior → novo`. Sem consequência, `Aplicar` aplica direto. | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md) |
| Menus (barra e contexto) | `MenuItem` com destaque arredondado a 4 px das bordas do menu; comandos em grafite, desabilitados em cinza claro; atalhos na fonte da interface, mais discretos; divisores recuados das laterais. Borda, cantos, fundo e sombra das superfícies flutuantes compartilhadas. | [0045](../docs/design/0045-controles-compartilhados-de-formularios-e-menus.md#apresentação-dos-itens) |
| `Exportar` na barra de menus | Botão principal comum de 22 px, cantos arredondados, centralizado na barra e afastado da borda; não ocupa a altura inteira da barra. | [0001](../docs/design/0001-estrutura-da-janela-do-projeto.md#menu-superior) |
...
| Visualizador de imagens em janela própria | Janela owned com superfície neutra quente e imagem inteira como foco. No encaixe, imagem proporcional e centralizada com 24 px de respiro em cima e embaixo e, nas laterais, o espaço das setas e das ferramentas (16 + 42 + 8 px), que ficam ao lado da imagem, não sobre ela. A barra da janela mostra somente o nome truncado e os controles nativos. Setas, olhos e ajuste usam ícones em grafite sobre círculos de 36 px com superfície clara translúcida (56% em repouso, 72% no hover), sem sombra. Alvo de 42 px e foco visível preservados. Estados transitórios ficam na área da imagem. Sem barra de ferramentas, contador ou porcentagem. | [0050](../docs/design/0050-visualizador-integrado-de-imagens.md) |
| Correção de olhos no visualizador | Abrir olhos fica no canto superior direito e se transforma em Fechar correção no mesmo ponto. Seu fundo circular se prolonga à esquerda em uma cápsula contínua translúcida com Salvar, sem discos separados, bordas, divisórias ou sombra; revelação discreta com movimento reduzido respeitado. Usar esta foto e Trocar referência ocupam a mesma cápsula flutuante com ícone e texto no centro inferior da metade esquerda, a 16 px da borda, sem rodapé nem tooltip. No mesmo ponto da metade direita, o botão de antes e depois nomeia `Original` e `Corrigida`, com a versão exibida sobre uma superfície elevada, e alterna com um clique. A barra da janela mostra `Corrigir olhos · <foto>`. Usa ImageToolButton (alvo de 42 px, ícones de 18 px, superfície de 36 px de altura, tooltip e foco acessíveis). Imagens inteiras e proporcionais ocupam as duas metades com 24 px de respiro (a referência reserva também as laterais para suas setas), sem nomes, rótulos ou faixa reservada para ferramentas. Selecionar os rostos no destino e na referência prepara a correção automaticamente. Antes e depois indica seu estado no próprio botão. Salvar abre a confirmação compartilhada Substituir foto original?, centralizada sobre as fotos, com Cancelar em foco; só Substituir original grava o resultado sobre o arquivo original. Esc cancela apenas a confirmação, preserva a prévia e devolve o foco a Salvar. Ao fechar o modo de correção, imagem e navegação continuam disponíveis no visualizador. Rostos recebem caixas de contorno dos pontos detectados, com cantos de 10 px, contorno branco e halo escuro fino para fotos claras e escuras, sem numeral nem preenchimento. O selecionado passa ao contorno azul de destaque e ganha um pequeno Check branco em disco azul integrado ao canto; caixas acompanham zoom e deslocamento, com aria-pressed. Sem mensagens soltas sobre as imagens: erros e ausência de rosto ficam apenas no tooltip da própria foto afetada, por hover ou foco de teclado, e fecham ao sair dela. Os botões mantêm somente as dicas de suas ações. Sem textos de andamento como Analisando, Preparando ou Aplicando correção; ocupação indicada pelos controles bloqueados e pelo estado acessível. | [0050](../docs/design/0050-visualizador-integrado-de-imagens.md) |
