...
| Correção de olhos no visualizador | Duas fotos com áreas de imagem iguais; decisões de referência na metade esquerda da mesma faixa inferior que alinha Cancelar e confirmação à direita. Progresso e erro ocupam linha reservada acima. Ferramentas de imagem usam `ImageToolButton` (alvo de 42 px; setas de 26 px, Eye/Scan de 18 px; tooltip React Aria com ajuste ao viewport; foco visível), compartilhado com o visualizador normal. Marcadores de rosto têm alvo de 42 px e núcleo visual de 20 px, deslocado dos olhos e com seleção clara. Aplicar usa `ActionButton` neutro; Cancelar e decisões de referência usam a variante quiet. | [0050](../docs/design/0050-visualizador-integrado-de-imagens.md) |
