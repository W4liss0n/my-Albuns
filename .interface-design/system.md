...
| Correção de olhos no visualizador | Duas fotos com áreas de imagem iguais; decisões de referência abaixo da foto esquerda, Cancelar e confirmação à direita. Ferramentas de imagem usam `ImageToolButton` (42 px, ícone com separação sobre a foto, tooltip, foco visível), compartilhado com o visualizador normal. Marcadores de rosto têm alvo de 42 px e núcleo visual de 20 px, deslocado dos olhos e com seleção clara. Aplicar usa `ActionButton` neutro; Cancelar e decisões de referência usam a variante quiet. | [0050](../docs/design/0050-visualizador-integrado-de-imagens.md) |
