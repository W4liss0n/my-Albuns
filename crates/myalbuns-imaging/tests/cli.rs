...
use myalbuns_core::{
    ...
    InitialProjectPersonalization, MediaKind, PhotoPlacementMode, PhotoSourceMetadata, ProjectCore,
    ProjectIntent, ProjectLocation, ProjectedFrameBorder, RectUm, RenderSnapshot,
    Rgb as ProjectRgb,
};
...
fn processor_renders_linked_original_pixels_and_only_the_configured_frame_border() {
    ...
    let mut snapshot = productive_photo_snapshot(&source_path, None);
    ...
    let source =
        RenderSource::new(photo.media_id, source_path).expect("the linked source is valid");
...
fn processor_composites_a_transparent_decorative_from_its_original_png() {
    ...
    let mut snapshot = productive_photo_snapshot(&photo_path, Some(&decorative_path));
    let overlay = snapshot.composition.sheets[0]
        .overlays
        .first()
        .cloned()
        .expect("the representative fixture contains an Overlay");
    let overlay_media_id = overlay.media_id;
    let sheet = &mut snapshot.composition.sheets[0];
    sheet.width_um = 25_400;
    sheet.height_um = 12_700;
    sheet.base.draw_rect.width = sheet.width_um;
    ...
    let (photo_media_id, draw_rect) = {
        ...
        photo.rotation_degrees = 0.0;
        photo.mirror_x = false;
        (photo.media_id, frame.clip_rect.clone())
    };
    ...
    let decorative_source = RenderSource::new(overlay_media_id, decorative_path.clone())
        ...
        small_initial_project(100).with_personalization(InitialProjectPersonalization::new(
            InitialBackground::BothSides {
                both: InitialBackgroundContent::Color {
                    rgb: ProjectRgb::parse_canonical("#FFFFFF")
                        .expect("the fixture Background color is canonical"),
                },
            },
            InitialOverlay::BothSides {
                both: overlay_path.map(|path| InitialOverlayContent::Media {
                    path: path.to_path_buf(),
                }),
            },
            InitialFrameBorder::None,
        ));
    let root = tempfile::tempdir().expect("temporary productive Photo Project directory");
    let project_path = root.path().join("ProcessorPhotoFixture.myalbuns");
    let mut context = OperationPathContext::new();
    context
        .capture(&project_path)
        .expect("the productive Photo Project root is captured");
    let mut project = ProjectCore::new()
        .with_identity_storage_roots(root.path().join("leases"), root.path().join("identities"))
        .create_editable(CreateProjectRequest::new(
            ProjectLocation::new(project_path, context.freeze()),
        ...
        .expect("the productive Photo Project is created through ProjectCore");
    let source_metadata = PhotoSourceMetadata::new(
        40,
        20,
        ["#F01010".into(), "#1020F0".into(), "#FFFFFF".into()],
    )
    .expect("the linked Photo metadata is valid");
