...
use crate::MediaKind;
mod decorations_v11;
mod media_folders_v12;
use media_folders_v12::ProjectDocumentV12;
mod layouts_v10;
use decorations_v11::ProjectDocumentV11;
mod layouts_v8;
mod layouts_v9;
...
use layouts_v10::ProjectDocumentV10;
...
pub(super) const SCHEMA_VERSION_V10: u32 = 10;
pub(super) const SCHEMA_VERSION_V11: u32 = 11;
pub(super) const SCHEMA_VERSION_V12: u32 = 12;
...
pub(super) fn decode(bytes: &[u8]) -> Result<DecodedProjectRevision, DecodeFailure> {
    ...
    if schema_version == SCHEMA_VERSION_V12 {
        let document: ProjectDocumentV12 = serde_json::from_slice(bytes)
    ...
    if schema_version == SCHEMA_VERSION_V11 {
        let document: ProjectDocumentV11 = serde_json::from_slice(bytes)
            .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
        return Ok(DecodedProjectRevision {
            revision: document.into_domain()?,
            source_schema_version: schema_version,
    ...
    if schema_version == SCHEMA_VERSION_V10 {
        let document: ProjectDocumentV10 = serde_json::from_slice(bytes)
            .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
        return Ok(DecodedProjectRevision {
            revision: document.into_domain()?,
            source_schema_version: schema_version,
...
pub(super) fn encode(revision: &ProjectRevision) -> Result<Vec<u8>, DecodeFailure> {
    ...
    let dto = ProjectDocumentV12::from_domain(revision)?;
...
pub(super) fn rewrite_project_id(
    ...
    let mut rewritten = match source.source_schema_version {
        SCHEMA_VERSION_V12 => {
            let mut document: ProjectDocumentV12 = serde_json::from_slice(bytes)
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
            document.project_id = project_id;
            serde_json::to_vec_pretty(&document)
        }
        SCHEMA_VERSION_V11 => {
            let mut document: ProjectDocumentV11 = serde_json::from_slice(bytes)
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
            document.project_id = project_id;
            serde_json::to_vec_pretty(&document)
        }
        SCHEMA_VERSION_V10 => {
            let mut document: ProjectDocumentV10 = serde_json::from_slice(bytes)
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
            document.project_id = project_id;
            serde_json::to_vec_pretty(&document)
        }
        ...
        SCHEMA_VERSION_V4 => {
            let mut document: ProjectDocumentV4 = serde_json::from_slice(bytes)
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
            document.project_id = project_id;
            serde_json::to_vec_pretty(&document)
        }
        ...
        SCHEMA_VERSION_V6 => {
            let mut document: ProjectDocumentV6 = serde_json::from_slice(bytes)
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?;
            document.project_id = project_id;
            serde_json::to_vec_pretty(&document)
        }
...
fn classify_header(bytes: &[u8]) -> Result<u32, DecodeFailure> {
    ...
    match version {
        ...
        | SCHEMA_VERSION_V9 | SCHEMA_VERSION_V10 | SCHEMA_VERSION_V11 | SCHEMA_VERSION_V12 => {
            Ok(version)
        }
...
fn map_document(document: ProjectDocumentV7) -> Result<ProjectRevision, DecodeFailure> {
    ...
    let sheets = document
        ...
        .map(map_sheet)
        .collect::<Result<Vec<_>, _>>()?;
    // The v7 -> v8 migration preserves every existing Frame and defaults to
    // allowing Pages and Sheet. No Last Layout is inferred from manual geometry.
    ...
    let photo = frame
        ...
        .map(|photo| {
            Ok(ProjectPhoto::new(
                ...
                .map_err(|_| document_failure(DocumentFailure::InvalidProjectDocument))?
                .with_black_and_white(photo.transform.black_and_white),
            ))
        })
        .transpose()?;
    Ok(ProjectFrame::new(
    ...
    fn from_domain(project: &ProjectDocument) -> Result<Self, DecodeFailure> {
        Ok(Self {
            ...
            sheets: project
                .sheets()
                .iter()
                .map(SheetWithFrames::from_domain)
                .collect(),
        })
    }
}
...
fn migrate_v3_to_v4(document: ProjectDocumentV3) -> Result<ProjectDocumentV4, DecodeFailure> {
...
fn migrate_v5_to_v6(document: ProjectDocumentV5) -> Result<ProjectDocumentV6, DecodeFailure> {
    if document.document_type != DOCUMENT_TYPE || document.schema_version != SCHEMA_VERSION_V5 {
        return Err(document_failure(DocumentFailure::InvalidProjectDocument));
    }
    Ok(
        document.map_transforms(SCHEMA_VERSION_V6, |old| PhotoTransformV6 {
            pan_x: old.pan_x,
            pan_y: old.pan_y,
            user_zoom: old.user_zoom,
            quarter_turns: old.quarter_turns,
            mirror_x: old.mirror_x,
            angle_tenths: old.angle_tenths,
...
fn migrate_v2_to_v3(document: ProjectDocumentV2) -> Result<ProjectDocumentV3, DecodeFailure> {
    ...
    Ok(ProjectDocumentV3 {
        ...
        project: FramedProjectPayload {
            ...
            visual_defaults,
            media,
            sheets: sheets.into_iter().map(SheetWithFrames::from_v2).collect(),
        },
    })
}
...
#[derive(Deserialize, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct FrameWithPhoto<Transform> {
    id: String,
    rect: RectV3,
...
#[derive(Deserialize, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct PhotoWithTransform<Transform> {
    media_id: String,
    transform: Transform,
}

impl PhotoWithTransform<PhotoTransformV6> {
    fn from_domain(photo: &ProjectPhoto) -> Self {
        Self {
            media_id: photo.media_id().hyphenated().to_string(),
            transform: PhotoTransformV6::from_domain(photo.transform()),
        }
    }
}
