...
impl RootBindingPlan {
    ...
    pub fn validate(&self) -> Result<(), AppPathsError> {
        for (index, binding) in self.bindings.iter().enumerate() {
            ...
            let (operational_root, _) = external_path_root(&binding.operational_root)?;
            if logical_root != binding.logical_root
                || operational_root != binding.operational_root
...
impl OperationPathContext {
    ...
    pub fn capture(&mut self, path: &Path) -> Result<(), AppPathsError> {
        ...
        self.capture_with_binding(path, &root)
    }
    ...
    pub fn capture_with_binding(
        ...
        let (validated_operational_root, _) = external_path_root(operational_root)?;
        if validated_operational_root != operational_root {
            return Err(AppPathsError::InvalidOperationPath);
        }
        if let Some(binding) = self
            ...
            .find(|binding| same_root(&binding.logical_root, &logical_root))
        {
            return if same_root(&binding.operational_root, &validated_operational_root) {
                Ok(())
            } else {
        ...
        self.bindings.push(RootBinding {
            kind,
            logical_root,
            operational_root: validated_operational_root,
        });
        Ok(())
    }
...
fn external_path_root(path: &Path) -> Result<(PathBuf, PathRootKind), AppPathsError> {
    ...
    let kind = match prefix_component.kind() {
        ...
        | Prefix::DeviceNS(_)
        | Prefix::UNC(_, _)
        | Prefix::VerbatimUNC(_, _) => return Err(AppPathsError::InvalidOperationPath),
    };
