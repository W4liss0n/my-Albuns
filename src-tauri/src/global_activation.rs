...
pub(crate) fn enter_global_activation(
    ...
    enter_global_activation_with_settings(app_paths, projects, None)
}

pub(crate) fn enter_global_activation_with_settings(
    app_paths: &AppPaths,
    projects: Vec<PathBuf>,
    settings: Option<SettingsSection>,
) -> io::Result<GlobalActivationEntry> {
...
mod tests {
    ...
    fn settings_activation_reuses_the_global_and_preserves_the_requested_section() {
        ...
        let paths = AppPaths::from_roots(&root.path().join("roaming"), &root.path().join("local"));
        let GlobalActivationEntry::Primary(primary) = enter_global_activation_with_settings(
            &paths,
            vec![],
        ...
        assert_eq!(
            primary.initial_settings(),
            Some(SettingsSection::Performance)
        );
        let forwarded_paths = paths.clone();
        let forwarder = std::thread::spawn(move || {
