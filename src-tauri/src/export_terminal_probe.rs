...
fn validate_probe_root(root: &Path) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "{EXPORT_TERMINAL_PROBE_ROOT_ENV} precisa apontar para uma pasta absoluta existente sob .scratch."
        ));
    }
    Ok(())
}
...
fn record_capture_failure(capture: &Arc<Mutex<ProbeCapture>>, reason: String) {
    let mut capture = capture
        .lock()
        .expect("the export terminal capture remains available");
