...
test("formats the opening time using local calendar days and pt-BR time", () => {
  ...
  expect(recentProjectOpeningTime(today.getTime(), now)).toEqual({
    label: "Hoje às 14:30", dateTime: today.toISOString(),
  });
  expect(recentProjectOpeningTime(yesterday.getTime(), now)?.label)
    .toBe("Ontem às 09:15");
  expect(recentProjectOpeningTime(older.getTime(), now)?.label)
    .toBe("18/09/2026 às 09:15");
});
...
test("uses calendar boundaries, including across the year, rather than elapsed hours", () => {
  ...
  expect(recentProjectOpeningTime(previousDay.getTime(), justAfterMidnight)?.label)
    .toBe("Ontem às 23:59");
});
