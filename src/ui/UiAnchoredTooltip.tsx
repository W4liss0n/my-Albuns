...
export function useUiAnchoredTooltip(targetRef: RefObject<HTMLElement | null>, label: string, disabled = false, preferredPlacement: Placement = "top") {
  ...
  const { overlayProps, placement } = useOverlayPosition({
    targetRef, overlayRef, isOpen: state.isOpen, placement: preferredPlacement,
    offset: 8, containerPadding: 12, shouldFlip: true,
  });
  return {
    triggerProps,
    tooltip: state.isOpen && createPortal(
      <span {...tooltipProps} {...overlayProps} ref={overlayRef} className="ui-anchored-tooltip ui-floating-tooltip" data-placement={placement}>
        {label}
      </span>, document.body,
    ),
  };
