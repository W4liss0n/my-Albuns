  ...
  return <OwnedWindowShell controls="close" context="Gerar projetos em lote" width={error !== null ? 520 : terminal && issues.length === 0 ? 400 : 800}>
    ...
    {!view ? <GenerationConfiguration model={model} port={port} busy={busy} onError={report} onClose={close}
      ...
      items={issues.map(item => ({
        key: item.id,
        title: item.name,
        titleHint: item.destination,
        details: terminal ? <>
          <span>{item.status === "ignored" ? "Ignorado" : item.status === "failed" ? "Falhou" : "Não gerado"}</span>
          {item.problems.length > 0 && <span>{item.problems.join(" ")}</span>}
      ...
      closeDisabled={busy} onClose={close}
      leadingActions={!terminal && <ActionButton disabled={busy} onClick={() => void perform(() => port.recheck())}>Verificar novamente</ActionButton>}
      primaryAction={!terminal && <ActionButton variant="primary" disabled={busy || !view.canContinue} onClick={() => void perform(() => port.run())}>Iniciar geração</ActionButton>} />}
    </div>
...
function GenerationConfiguration({ model, port, busy, onSubmit, onError, onClose }: {
  ...
  return <div className="ui-operation-dialog ui-export-dialog generation-configuration"><DialogWindowFrame title="Gerar projetos em lote" layout="form" actions={<>
    ...
    <form className="ui-operation-form" onSubmit={event => event.preventDefault()} aria-busy={busy}>
      <section className="ui-operation-form__section generation-model" aria-labelledby={`${id}-model`}>
        <h3 className="generation-model__title" id={`${id}-model`}>Modelo</h3>
        <div className="generation-model__identity">
          <strong title={model}>{model || "\u00a0"}</strong>
          <p>Inclui as alterações ainda não salvas. O modelo permanece inalterado.</p>
        </div>
      </section>
      <fieldset className="ui-operation-form__section" disabled={busy}><legend title="Cada pasta com fotos gera um projeto, incluindo subpastas. As novas fotos entram somente no painel.">Origem</legend>
        <div className="ui-operation-form__destination"><TextInput className="ui-field-control" aria-label="Pasta de origem" placeholder="Selecione a pasta com as fotos" value={source} title={source} onChange={event => setSource(event.target.value)} /><ActionButton onClick={() => void choose("source")}>Escolher…</ActionButton></div>
      </fieldset>
      <fieldset className="ui-operation-form__section" disabled={busy}><legend title="A organização das pastas será mantida. Escolha um destino fora da pasta de origem e de suas subpastas.">Destino</legend>
        <div className="ui-operation-form__destination"><TextInput className="ui-field-control" aria-label="Pasta de destino" placeholder="Selecione onde salvar os projetos" value={destination} title={destination} onChange={event => setDestination(event.target.value)} /><ActionButton onClick={() => void choose("destination")}>Escolher…</ActionButton></div>
      </fieldset>
    </form>
