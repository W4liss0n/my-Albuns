...
function FaceImage({ side, url, name, choosing, selected, onChoose }: {
  ...
  return <div className="eye-correction__pane-image" ref={pane} tabIndex={0}
    ...
    {choosing && faces.map((face, index) => {
      ...
      return <button key={index} type="button" className="eye-correction__face" data-selected={selected === index}
        style={{ left: size.width / 2 + ((center.x - 0.5) * imageWidth) * zoom + pan.x, top: size.height / 2 + ((center.y - 0.5) * imageHeight) * zoom + pan.y }}
        title={`Selecionar rosto ${index + 1}`} aria-label={`${side === "reference" ? "Referência" : "Imagem a corrigir"}: rosto ${index + 1}`}
        onClick={() => onChoose(index, faces)}><span>{index + 1}</span></button>;
    })}
    {choosing && message && <p className="eye-correction__status" role="status">{message}</p>}
    {zoom > 1 && <button type="button" className="eye-correction__fit" onClick={fit} aria-label={`Ajustar ${side === "reference" ? "referência" : "imagem a corrigir"} à janela`}>Ajustar</button>}
  </div>;
}
    ...
    <div className="eye-correction__panes">
      <div className="eye-correction__pane">
        ...
        <FaceImage side="reference" url={correction.referenceUrl} name={correction.referenceName} choosing={referenceChoosing}
          selected={referenceIndex} onChoose={(index, faces) => { setReferenceIndex(index); setReferenceFaces(faces); }} />
        {correction.phase === "browse" && <>
          <button type="button" className="eye-correction__nav eye-correction__nav--previous" aria-label="Referência anterior" disabled={!correction.canPreviousReference} onClick={() => onNavigate(-1)}><AppIcon icon={ChevronLeft} size={18} /></button>
          <button type="button" className="eye-correction__nav eye-correction__nav--next" aria-label="Próxima referência" disabled={!correction.canNextReference} onClick={() => onNavigate(1)}><AppIcon icon={ChevronRight} size={18} /></button>
        </>}
      </div>
      <div className="eye-correction__pane">
        ...
        <FaceImage side="target" url={(correction.phase === "preview" || correction.phase === "applying") && correction.resultUrl ? correction.resultUrl : presentation.url}
          ...
          onChoose={(index, faces) => { setTargetIndex(index); setTargetFaces(faces); }} />
      </div>
    </div>
    <div className="eye-correction__footer">
      <p className="eye-correction__hint" role="status">{correction.error ?? (
        correction.phase === "browse" ? "Selecione o rosto a corrigir e escolha outra foto do projeto com os olhos abertos." :
        correction.phase === "select" ? "Selecione um rosto em cada foto." :
        correction.phase === "processing" ? "Preparando correção…" :
        correction.phase === "applying" ? "Aplicando correção…" : "Confira o resultado antes de aplicar."
      )}</p>
      <div className="eye-correction__actions">
        <button type="button" disabled={correction.phase === "applying"} onClick={() => action("cancel")}>Cancelar</button>
        {(correction.phase === "select" || correction.phase === "preview") && <button type="button" onClick={() => action("browse")}>Outra referência</button>}
        {correction.phase === "browse" && <button type="button" disabled={!correction.referenceUrl || correction.referenceState !== "ready"} onClick={() => action("select")}>Usar como referência</button>}
        {correction.phase === "select" && <button type="button" disabled={targetIndex === null || referenceIndex === null}
          onClick={() => onCorrection({ sessionId: presentation.sessionId, kind: "preview", referenceMediaId: correction.referenceMediaId, targetFace: targetFaces[targetIndex!], referenceFace: referenceFaces[referenceIndex!] })}>Ver correção</button>}
        {correction.phase === "preview" && <button type="button" onClick={() => action("apply")}>Aplicar</button>}
      </div>
    </div>
  </div>;
}
