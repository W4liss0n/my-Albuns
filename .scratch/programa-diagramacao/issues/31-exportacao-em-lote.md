...
- [ ] Ao continuar, adquirir um único `OperationLease` exclusivo, que conserva junto a concessão do `OperationGate`, a pausa do `CacheEngine` e a reserva do Processador durante todo o lote; bloquear interação em todas as Janelas de Projeto e manter interativa somente a janela global de progresso/cancelamento. A política de lote pertence ao `BatchRunner`, não a um rótulo do lease.
